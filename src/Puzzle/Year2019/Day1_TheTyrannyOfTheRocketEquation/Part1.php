...
class Part1 implements PuzzleInterface
{
    ...
    public function solution($moduleMassesMultilineString)
    {
        ...
        $moduleMasses = $this->parseModuleMassesMultilineString($moduleMassesMultilineString);

        // Calculate total fuel needed for spacecraft
        foreach ($moduleMasses as $moduleMass) {
            $totalFuel += $this->calculateFuelForModule($moduleMass);
        }
    ...
    {
        $moduleMassesMultilineString = str_replace("\t", " ", $moduleMassesMultilineString);
        $rows = preg_split("/(\n)/", $moduleMassesMultilineString);

        $moduleMasses = array_map('intval', $rows);
    ...
    {
        $moduleFuel = (int) floor($moduleMass / 3) - 2;
