...
namespace App\Puzzle\Year2019\Day2_1202ProgramAlarm;

use App\Puzzle\PuzzleInterface;

class Part1 implements PuzzleInterface
{
    private const FIRST_POSITION_REPLACEMENT = 12;
    ...
    public function solution($intcode)
    {
        ...
        $intcodeArray = $this->parseIntcode($intcode);
        $intcodeArray = $this->replaceFirstAndSecondPositionsWithInputs(
            $intcodeArray,
            self::FIRST_POSITION_REPLACEMENT,
            self::SECOND_POSITION_REPLACEMENT
        );

        $outputIntcodeArray = $this->getOutputIntcodeArray($intcodeArray);

        $position0 = $outputIntcodeArray[0];
    ...
    {
        $intcodeArray = explode(',', trim($intcode));
        $intcodeArray = array_map('intval', $intcodeArray);

        return $intcodeArray;
    }

    /**
     * Replaces Intcode array on first and second positions.
     *
    ...
    {
        $intcodeArray[1] = $first;
        $intcodeArray[2] = $second;
    ...
    {
        // Initialize Opcode and indexer
        $opcode = $intcodeArray[0];
        $index = 0;

        // Program is finished and should immediately halt on Opcode 99
        while ($opcode != 99) {
            // Get input and output positions
            $input1Position = $intcodeArray[$index + 1];
            $input2Position = $intcodeArray[$index + 2];
            $outputPosition = $intcodeArray[$index + 3];

            // Get input values
            $input1 = $intcodeArray[$input1Position];
            $input2 = $intcodeArray[$input2Position];

            // Do operation based on Opcode, and overwrite value at output position
            if ($opcode === 1) {
                $intcodeArray[$outputPosition] = $input1 + $input2;
            } elseif ($opcode === 2) {
                $intcodeArray[$outputPosition] = $input1 * $input2;
            }

            // Once Opcode is processed, move forward 4 positions
            $index = $index + 4;
            // Find new Opcode
            $opcode = $intcodeArray[$index];
        }
