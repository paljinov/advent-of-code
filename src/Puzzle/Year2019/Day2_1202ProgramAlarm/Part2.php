...
O"Good, the new computer seems to be working correctly! Keep it nearby during
...
class Part2 implements PuzzleInterface
{
    ...
    public function solution($intcode)
    {
        ...
        $intcodeArray = $this->part1->parseIntcode($intcode);

        // Each of the two input values can be between 0 and 99
        $limit = 99;
        for ($noun = 0; $noun < $limit; $noun++) {
            for ($verb = 0; $verb < $limit; $verb++) {
                $intcodeArrayForNounAndVerb = $this->part1
                    ->replaceFirstAndSecondPositionsWithInputs($intcodeArray, $noun, $verb);
                $outputIntcodeArray = $this->part1->getOutputIntcodeArray($intcodeArrayForNounAndVerb);
                if ($outputIntcodeArray[0] === self::OUTPUT) {
                    break 2;
                }
            }
        }
